...
public class RepairShopConfig {
    ...
    Properties getProps() {
        ...
        try{
            System.out.println("Searching db.config file in: " + new File(".").getAbsolutePath());
            props.load(new FileReader("bd.config"));
        }
        catch (IOException e){
            System.out.println("Cannot find bd.config file");
            e.printStackTrace();
        }
        finally {
            System.out.println("Properties Found: " + props);
        }
        return props;
    }
