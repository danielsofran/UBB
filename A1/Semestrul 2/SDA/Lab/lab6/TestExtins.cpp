...
void testSterge() {
	...
	for (int i = -100; i < -10; i++) { //stergem o aparitie de la fiecare
		assert(c.sterge(i) == true);
	}
    for (int i = -10; i < 100; i++) { //stergem o aparitie de la fiecare
        assert(c.sterge(i) == true);
    }
	assert(c.dim() == 800);
	for (int i = -100; i < 100; i++) {
...
void testFunctionalitate(){
    ...
    c.adauga(3);
    c.adauga(3);
    assert(c.elementeCuFrecventaData(0) == 0);
...
void testAllExtins() {
	testCreeaza();
    cout<<"Test Creeaza gata!\n";
	testAdauga();
    cout<<"Test Adauga gata!\n";
	testSterge();
    cout<<"Test Sterge gata!\n";
	testIterator();
    cout<<"Test Iterator gata!\n";
    testFunctionalitate();
    cout<<"Test Functionalitate gata!\n";
	testQuantity();
}