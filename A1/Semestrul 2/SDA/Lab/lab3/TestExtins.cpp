...
bool par(TElem elem) { return elem % 2 == 0; }
bool impar(TElem elem) { return elem % 2 != 0; }
...
void testAllExtins() {
    ...
    // numar elementele pare si impare
    int pare = 0, impare = 0;
    it0.prim();
    while(it0.valid())
    {
        if(par(it0.element())) pare++;
        else impare++;
        it0.urmator();
    }
    lista.adaugaInceput(1); // va trebui sters
    lista.adaugaSfarsit(5); // va trebui sters
    impare += 2;

    // testare functionalitate noua
    lista.filtreaza(par);
    assert(lista.dim() == pare);
    it0.prim();
    while(it0.valid())
        assert(par(it0.element())),
        it0.urmator();

    lista.filtreaza(impar);
    assert(lista.dim()==0);
    assert(lista.vida());
}
