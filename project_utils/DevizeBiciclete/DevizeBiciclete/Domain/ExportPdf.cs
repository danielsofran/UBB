...
namespace DevizeBiciclete.Domain
{
    public static class ExportPdf
    {
        ...
        public static void ToPDF(this DevizData devizData, string path, int enters=1)
        {
            ...
            initFragmentSubtitluSt(out fragmentSubtitlu);
            fragmentSubtitlu.Text = "Bicicleta";
            page.Paragraphs.Add(fragmentSubtitlu);

            initFragmentText(out fragment);
            //fragment.HorizontalAlignment = Aspose.Pdf.HorizontalAlignment.Center;
            ...
            if(table.Rows[0].IsInNewPage || enters>0) // automat?
            {
                TextFragment enterp = new TextFragment(n);
                enterp.TextState.FontSize = fragment.TextState.FontSize;
                for(int i=1;i<=enters;++i) page.Paragraphs.Add(enterp);
            }
            page.Paragraphs.Add(fragment);
