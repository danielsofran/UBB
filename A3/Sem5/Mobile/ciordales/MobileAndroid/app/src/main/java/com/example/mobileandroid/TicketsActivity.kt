...
class TicketsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val notificationManager = getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager
        // Notification ID allows updating or canceling the notification later
        ...
        textView.text = isOnline(context)
            ...
            override fun onResponse(call: Call<List<TicketDto>>, response: Response<List<TicketDto>>) {
                if (response.isSuccessful) {
                    ...
                    val adapter = TicketAdapter(ticketsArray.toMutableList()) { ticket ->
                        //Toast.makeText(this@TicketsActivity, "Ticket ${ticket.name} clicked", Toast.LENGTH_SHORT).show()
                        val intent = Intent(this@TicketsActivity, EditTicketActivity::class.java)
                        Toast.makeText(this@TicketsActivity, "Intent created", Toast.LENGTH_SHORT).show()
                        intent.putExtra("ticket", ticket)
                        startActivityForResult(intent, REQUEST_EDIT_TICKET)
                        //Toast.makeText(this@TicketsActivity, "Edit Ticket ${ticket.name} clicked", Toast.LENGTH_SHORT).show()
                    }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        log.warning("tickets onActivityResult")
