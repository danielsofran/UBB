...
class ItemRepository(
    ...
    suspend fun saveLocal(item: Item): Item {
        Log.d(TAG, "save $item...")
        item._id = generateId(12)
        handleItemCreated(item)
        Log.d(TAG, "locally save $item locally succeeded")
        return item
    }

    suspend fun updateLocal(item: Item): Item {
    ...
    suspend fun deleteItem(item: Item) {
        itemDao.deleteItem(item.make,item.model, item.year)
    }
